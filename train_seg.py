...
def train(rank, world_size, args):
    try:
        ...
        if restore_ckpt is not None:
            ...
            model__state_dict = AsymKD_VIT.state_dict()
            new_state_dict = {}
            for k, v in checkpoint.items():
                # 키 매핑 규칙을 정의
                new_key = k # 'module.'를 제거
                if new_key in model__state_dict:
                    new_state_dict[new_key] = v

            model__state_dict.update(new_state_dict)
            AsymKD_VIT.load_state_dict(model__state_dict)
        ...
        while should_keep_training:
            ...
            for i_batch, data_blob in enumerate(tqdm(train_loader)):
                ...
                if epoch >= 0 and total_steps % save_step == save_step-1 and rank == 0:
                    save_path = Path('checkpoint_seg_proj/%d_%s.pth' % (total_steps + 1, args.name))
            ...
            if len(train_loader) >= 10000:
                save_path = Path('checkpoint_seg_proj/%d_epoch_%s.pth' % (total_steps + 1, args.name))
        ...
        logger.close()
        PATH = 'checkpoint_seg_proj/%s.pth' % args.name
        torch.save({
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
            'scheduler_state_dict': scheduler.state_dict(),
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', type=int, default=5, help="length of training schedule.")
    ...
    Path("checkpoint_seg_proj").mkdir(exist_ok=True, parents=True)
