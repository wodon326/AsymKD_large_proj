...
def train(rank, world_size, args):
    try:
            ...
            for i_batch, data_blob in enumerate(tqdm(train_loader)):
                ...
                if rank == 0:
                    ...
                    if total_steps % save_step == save_step-1:
                        save_path = Path(f"checkpoint_kd_wavelet/{total_steps + 1}_{args.name}.pth")
        ...
        logger.close()
        state.save(f"checkpoint_kd_wavelet/{args.name}.pth")
        return None
    finally:
        cleanup()
...
if __name__ == '__main__':
    ...
    Path("checkpoint_kd_wavelet").mkdir(exist_ok=True, parents=True)
