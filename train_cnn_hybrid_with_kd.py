...
class Logger:
    ...
    def upload(self, model_type: str, total_samples: int, total_steps: int):
        ...
        for k in self.metric_history[model_type]:
            self.add_scalar(f"metrics_kd/{k}/{model_type}", self.metric_history[model_type][k]/total_samples, total_steps)
...
def train(rank, world_size, args):
    try:
        ...
        logger = Logger(rank, args.save_dir, keys=["val_student", "val_small", "val_compress", "train_student"])
        ...
        while total_steps < args.num_steps:
            for i_batch, data_blob in enumerate(pbar := tqdm(train_loader)):
                ...
                if rank == 0 and total_steps % 250 == 250 - 1:
                    ...
                    for model_type in ["val_student", "val_small", "val_compress"]:
