...
from AsymKD.kd_naive_dpt_latent1_avg_ver_all_loss import AsymKD_kd_naive_latent1_avg_ver_all_loss
...
def train(rank, world_size, args):
    try:
        ...
        # load model
        AsymKD_naive_kd = AsymKD_kd_naive_latent1_avg_ver_all_loss().to(rank)
        ...
        while total_steps < args.num_steps:
            for i_batch, data_blob in enumerate(pbar := tqdm(train_loader)):
                ...
                flow_predictions, student_features = model(depth_image)
                assert model.training
                ...
                if rank == 0:
                    logger.add_scalar("total_feature_loss", loss.item(), total_steps)
                    for i, feature_loss in enumerate(loss_arr):
                        logger.add_scalar(f"feature_loss/layer{i}", feature_loss.item(), total_steps)
