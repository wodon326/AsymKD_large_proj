        ...
        try:
            ...
            if self.gt:
                ...
                if index in self.cache.keys():
                    img1 = self.cache[index]
                    # print('load_from_cache')
                else:
                    img1 = cv2.imread(self.image_list[index])
                    self.cache[index] = img1
                    # print('save to cache')
...
class ImageNet_1k(StereoDataset):
    def __init__(self, aug_params=None, root='../../data/AstmKD'):
...
class VKITTI(StereoDataset):
    def __init__(self, aug_params=None, root='../../datasetssets/vkitti'):
...
class Hypersim(StereoDataset):
    def __init__(self, aug_params=None, root='../../datasetssets/Hypersim'):
...
def fetch_dataloader(args, rank, world_size):
    ...
    # dataset_size = len(train_dataset)
    # train_size = int(0.0005 * dataset_size)
    # val_size = dataset_size - train_size
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, drop_last=True, collate_fn=collate_fn, num_workers = 0, sampler = sampler)
    val_loader = data.DataLoader(val_dataset, batch_size=1, drop_last=False, collate_fn=collate_fn_gt, num_workers = 4, pin_memory=True)
