...
class Augmentor_with_no_gt:
    ...
    def spatial_transform(self, img1):
...
class Augmentor_with_gt:
    ...
    def spatial_transform(self, img1, flow, valid):
        # randomly sample scale
        ...
        flow = flow[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        valid = valid[y0:y0+self.crop_size[0], x0:x0+self.crop_size[1]]
        return img1, flow, valid


    def __call__(self, img1, flow, valid):
        # img1, img2 = self.color_transform(img1, img2)
        # img1, img2 = self.eraser_transform(img1, img2)
        img1, flow, valid = self.spatial_transform(img1, flow, valid)
