...
class DinoVisionTransformer(nn.Module):
    ...
    def _get_intermediate_layers_not_chunked_start_intermediate(self, x, n=1):
        # If n is an int, take the n last blocks. If it's a list, take them
        output, total_block_len = [], len(self.blocks)
        for i, blk in enumerate(self.blocks[total_block_len - n:]):
            x = blk(x)
            output.append(x)
        assert len(output) == n, f"only {len(output)} / {n} blocks found"
        return output
    ...
    def get_intermediate_layers_with_cls_token(
        self,
        x: torch.Tensor,
        n: Union[int, Sequence] = 1,  # Layers or n last layers to take
        reshape: bool = False,
    ...
    def get_first_intermediate_layers(
        self,
        x: torch.Tensor,
        n: Union[int, Sequence] = 1,  # Layers or n last layers to take
    ...
    def get_first_intermediate_layers_seg(
        ...
        output = self._get_first_intermediate_layers_not_chunked(x, n)
        # print(f'output[:, 1 + self.num_register_tokens:] {output[:, 1 + self.num_register_tokens:].shape} self.num_register_tokens {self.num_register_tokens}')
        return output[:, 1 + self.num_register_tokens:]


    def get_intermediate_layers_start_intermediate(
        self,
        x: torch.Tensor,
        n: Union[int, Sequence] = 1,  # Layers or n last layers to take
        reshape: bool = False,
