...
class Diffusion_dpt_latent1_avg_ver(nn.Module):
    ...
    def forward(self, x):
        ...
        batch_size, _, channels = student_intermediate_feature.shape
        cond_feature = rearrange(student_intermediate_feature, 'b (h w) c -> b c h w', h=patch_h, w=patch_w)
        knowledge_feature = self.feature_generate_diffusion.sample(cond_feature, shape=(batch_size, channels, patch_h, patch_w))
        knowledge_feature = rearrange(knowledge_feature, 'b c h w -> b (h w) c')
