...
class Residual_cbam_transconv_Adapter(nn.Module):
    def __init__(self, output_channels=384):
        super(Residual_cbam_transconv_Adapter, self).__init__()
        
        self.conv_interpolate4 = nn.Sequential(
            nn.Conv2d(
        ...
        self.conv_interpolate2 = nn.Sequential(
            nn.Conv2d(
                in_channels=output_channels,
        ...
        self.conv_interpolate1 = nn.Sequential(
            nn.Conv2d(
                in_channels=output_channels,
                out_channels=output_channels//2,
                kernel_size=1,
        ...
        self.fusion_refinenet1 = _make_fusion_block(output_channels//4, True)
        self.fusion_refinenet2 = _make_fusion_block(output_channels//4, True)
        self.fusion_refinenet3 = _make_fusion_block(output_channels//4, True)
        self.residual_blocks1 = Residual_Blocks(channels=output_channels//4, num_blocks=2)
        self.cam1 = ChannelAttentionEnhancement(output_channels//4)
        self.sam1 = SpatialAttentionExtractor()
        self.downsample1 = nn.Sequential(
                nn.Conv2d(output_channels//4, output_channels//2, kernel_size=3, stride=2, padding=1),
                nn.BatchNorm2d(output_channels//2),
                nn.ReLU()
            )
        

        self.residual_blocks2 = Residual_Blocks(channels=output_channels//2, num_blocks=2)
        self.cam2 = ChannelAttentionEnhancement(output_channels//2)
        self.sam2 = SpatialAttentionExtractor()
        self.downsample2 = nn.Sequential(
                nn.Conv2d(output_channels//2, output_channels, kernel_size=3, stride=2, padding=1),
                nn.BatchNorm2d(output_channels),
                nn.ReLU()
            )

        self.residual_blocks3 = Residual_Blocks(channels=output_channels, num_blocks=2)
    ...
    def forward(self, x, h, w, patch_h, patch_w):
        ...
        x_4 = self.conv_interpolate4(x)
        x_2 = self.conv_interpolate2(x)
        x_1 = self.conv_interpolate1(x)

        x_1 = self.fusion_refinenet1(x_1, size=x_2.shape[2:])
        x_2 = self.fusion_refinenet1(x_1, x_2)
        x = self.fusion_refinenet2(x_2, x_4)

        # x = F.interpolate(x, size=(h//4, w//4), mode='bilinear', align_corners=False)
        # x = self.conv(x)
        # print(f'fusion_refinenet2 : {x.size()}')
        cam_feat = self.cam1(x) * x
        attn = self.sam1(cam_feat)
        x = x * attn + x * (1 - attn)
        x = self.residual_blocks1(x)
        x = self.downsample1(x)
        # print(f'downsample1: {x.size()}')
        ...
        cam_feat = self.cam3(x) * x
        attn = self.sam3(cam_feat)
        x = x * attn + x * (1 - attn)
        x = self.residual_blocks3(x)
        x = self.downsample3(x)

        # print(f'downsample2: {x.size()}')
        x = self.final_conv(x)
