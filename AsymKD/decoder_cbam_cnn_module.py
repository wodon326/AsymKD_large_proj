...
class decoder_cbam_Adapter(nn.Module):
    def __init__(self, output_channels=64):
        super(decoder_cbam_Adapter, self).__init__()
        
        self.conv = nn.Sequential(
            nn.Conv2d(output_channels, output_channels, kernel_size=3, padding=1),
            nn.ReLU()
        )
        self.residual_blocks1 = Residual_Blocks(channels=output_channels*3, num_blocks=2, stride=1)
        self.cam1 = ChannelAttentionEnhancement(output_channels)
        self.sam1 = SpatialAttentionExtractor()

        self.channel_conv = nn.Conv2d(output_channels*3, output_channels, kernel_size=3, padding=1)
        self.relu = nn.ReLU()

        self.residual_blocks2 = Residual_Blocks(channels=output_channels, num_blocks=6, stride=1)
    ...
    def forward(self, x):
        ...
        x_high_freq = x * attn
        x_low_freq = x * (1 - attn)
        x = torch.cat([x_high_freq, x_low_freq, x], dim=1)
...
class decoder_Adapter(nn.Module):
    def __init__(self, output_channels=64):
        super(decoder_Adapter, self).__init__()
        ...
        self.residual_blocks = Residual_Blocks(channels=output_channels, num_blocks=12, stride=1)
