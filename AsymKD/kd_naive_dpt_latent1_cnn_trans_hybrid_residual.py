...
from AsymKD.unet_cnn_module import UNet_CBAM_adapter_for_cnn_trans_hybrid
import math
...
class AsymKD_kd_naive_dpt_latent1_cnn_trans_hybrid_residual(nn.Module):
    def __init__(self, encoder='vits', features= 64, out_channels= [48, 96, 192, 384], use_bn=False, use_clstoken=False, localhub=True):
        super(AsymKD_kd_naive_dpt_latent1_cnn_trans_hybrid_residual, self).__init__()
        
        assert encoder in ['vits', 'vitb', 'vitl']
        print('AsymKD_kd_naive_dpt_latent1_cnn_trans_hybrid_residual')
        ...
        self.unet_cbam_adapter = UNet_CBAM_adapter_for_cnn_trans_hybrid(base_channels=64, output_channels=128)
        ...
        self.num_patches, self.interpolate_offset, self.interpolate_antialias = self.pretrained.get_pos_embedding_variable()
        self.pos_embedding_for_adapter = nn.Parameter(torch.zeros(1, self.num_patches, in_channels_384))
    ...
    def interpolate_pos_encoding_for_adapter(self, x, w, h):
        previous_dtype = x.dtype
        npatch = x.shape[1]
        N = self.pos_embedding_for_adapter.shape[1]
        if npatch == N and w == h:
            return self.pos_embedding_for_adapter
        patch_pos_embed = self.pos_embedding_for_adapter.float()
        dim = x.shape[-1]
        w0 = w // 14
        h0 = h // 14
    ...
    def forward(self, x):
        ...
        uent_cbam_feature = uent_cbam_feature + self.interpolate_pos_encoding_for_adapter(uent_cbam_feature,w,h)
        uent_cbam_feature = self.refine_layers_Self1(uent_cbam_feature)
        uent_cbam_feature = self.refine_layers_Self2(uent_cbam_feature)
        uent_cbam_feature = self.refine_layers_Self3(uent_cbam_feature)


        sum_feat = uent_cbam_feature + student_intermediate_feature
