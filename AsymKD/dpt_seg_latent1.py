...
class AsymKD_seg_compress(nn.Module):
    def __init__(self, ImageEncoderViT, encoder='vits', features= 64, out_channels= [48, 96, 192, 384], use_bn=False, use_clstoken=False, localhub=True):
        super(AsymKD_seg_compress, self).__init__()
        
        assert encoder in ['vits', 'vitb', 'vitl']
        
        self.ImageEncoderViT = ImageEncoderViT.eval()
    ...
    def forward(self, x, y):
        ...
        seg_feature = self.ImageEncoderViT(y)
        student_intermediate_feature = self.pretrained.get_first_intermediate_layers_seg(x, 5)
        ...
        channel_proj_feat = self.Projects_layers_Channel_based_CrossAttn_Block(student_intermediate_feature,seg_feature)
        feat = self.Projects_layers_Cross(student_intermediate_feature,channel_proj_feat)
        feat = self.Projects_layers_Self(feat)
        features = self.pretrained.get_intermediate_layers_start_intermediate_seg(feat, 4)
