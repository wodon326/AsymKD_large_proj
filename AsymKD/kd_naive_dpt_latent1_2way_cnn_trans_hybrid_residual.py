...
from AsymKD.unet_cnn_module import UNet_CBAM_adapter_for_trans
...
class AsymKD_kd_naive_dpt_latent1_2way_cnn_trans_hybrid_residual(nn.Module):
    def __init__(self, encoder='vits', features= 64, out_channels= [48, 96, 192, 384], use_bn=False, use_clstoken=False, localhub=True):
        super(AsymKD_kd_naive_dpt_latent1_2way_cnn_trans_hybrid_residual, self).__init__()
        
        assert encoder in ['vits', 'vitb', 'vitl']
        print('AsymKD_kd_naive_dpt_latent1_2way_cnn_trans_hybrid_residual')
        ...
        self.unet_cbam_adapter = UNet_CBAM_adapter_for_trans(output_channels=dim)
    ...
    def forward(self, x):
        ...
        sum_feat = self.refine_layers_Self1(sum_feat)
        sum_feat = self.refine_layers_Self2(sum_feat)
        sum_feat = self.refine_layers_Self3(sum_feat)
