...
class AsymKD_kd_naive_latent1_avg_ver_no_cls(nn.Module):
    def __init__(self, encoder='vits', features= 64, out_channels= [48, 96, 192, 384], use_bn=False, use_clstoken=False, localhub=True):
        super(AsymKD_kd_naive_latent1_avg_ver_no_cls, self).__init__()
        
        assert encoder in ['vits', 'vitb', 'vitl']
        print('AsymKD_kd_naive_latent1_avg_ver_no_cls')
    ...
    def forward(self, x):
        ...
        intermediate_feature = self.pretrained.get_first_intermediate_layers_no_cls_token(x, 4)
    ...
    def forward_return_all_feat(self, x):
        ...
        if self.training:
            return depth, blocks_feat

        return depth
    
    def freeze_kd_naive_dpt_latent1_with_kd_style(self):
