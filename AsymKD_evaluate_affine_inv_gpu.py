...
if "__main__" == __name__:
    ...
    if "depth_anything" in model_type:
        ...
        restore_ckpt = "checkpoints_new_loss_001_smooth/62000_AsymKD_new_loss.pth"
    ...
    for data in tqdm(dataloader, desc="Evaluating"):
        ...
        if model_type == "metric3d":
            ...
            # elif "eth3d" in dataset_config:
            #     pred_size = (518, 770)
            if len(rgb.shape) == 3:  # (C, H, W)
                c, h, w = rgb.shape
            elif len(rgb.shape) == 4:  # (N, C, H, W)
                n, c, h, w = rgb.shape
            else:
                raise ValueError("Input image must be of shape (C, H, W) or (N, C, H, W)")

            # Calculate the scaling factor
            if h < w:  # Shorter side is height
                scale_factor = 518 / h
            else:  # Shorter side is width
                scale_factor = 518 / w
            ...
            # Adjust dimensions to be divisible by 14
            new_h = (new_h // 14) * 14 if new_h % 14 == 0 else ((new_h // 14) + 1) * 14
            new_w = (new_w // 14) * 14 if new_w % 14 == 0 else ((new_w // 14) + 1) * 14

            pred_size = (new_h, new_w)
