...
from AsymKD.kd_naive_decoder_cbam_dpt_latent1_avg_ver import Asymkd_decoder_cbam_kd_naive_dpt_latent1
from AsymKD.kd_naive_decoder_dpt_latent1_avg_ver import Asymkd_decoder_kd_naive_dpt_latent1
...
def train(rank, world_size, args):
    try:
        ...
        # load model
        AsymKD_naive_kd = Asymkd_decoder_cbam_kd_naive_dpt_latent1().to(rank)
        new_state_dict = AsymKD_naive_kd.load_ckpt(ckpt, device=torch.device('cuda', rank))
        if rank == 0:
            logging.info(f"loading backbones from {ckpt}")
            print('AsymKD_naive_kd : ', new_state_dict.keys())
        AsymKD_naive_kd.freeze_kd_naive_decoder_cbam_dpt_latent1_with_kd_style()
        ...
        while total_steps < args.num_steps:
            for i_batch, data_blob in enumerate(pbar := tqdm(train_loader)):
                ...
                with torch.no_grad():
                    teacher_prediction, teacher_feature = AsymKD_Compress.forward_with_path_1(depth_image)
